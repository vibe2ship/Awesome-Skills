...
| Lab | Generated by | Description |
|-----|-------------|-------------|
| [`mini-lsm`](labs/mini-lsm/) | `/mentor` | A pedagogical LSM-Tree storage engine in Rust. Covers MemTable, SSTable, WAL, Bloom Filter, compaction strategies, and merge iterators. 8 milestones from core types to a fully integrated engine. |
...
| 项目 | 生成工具 | 说明 |
|------|---------|------|
| [`mini-lsm`](labs/mini-lsm/) | `/mentor` | 教学向的 LSM-Tree 存储引擎（Rust）。涵盖 MemTable、SSTable、WAL、Bloom Filter、Compaction 策略和 Merge Iterator。共 8 个里程碑，从核心类型到完整引擎逐步推进。 |
